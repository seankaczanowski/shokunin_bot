...
async function getSpecifier(currentSpecifier, suggestedSpecifier) {
    ...
    if (!suggestedSigil) {
        ...
        alternativeSpecifiers.sort((a, b) => {
            var _a, _b;
            const [, , v1] = (_a = a.match(SEMVER_RANGE)) !== null && _a !== void 0 ? _a : [];
            const [, , v2] = (_b = b.match(SEMVER_RANGE)) !== null && _b !== void 0 ? _b : [];
...
async function getIndividualVersion(pkg, specifier) {
    var _a;
...
async function getVersion(pkg, specifier) {
    ...
    specifier = (await Promise.all(specifier
        .split('||')
        .map(specifier => specifier.trim())
        .map(async (specifier) => getIndividualVersion(pkg, specifier)))).join(' || ');
